...
class Tokenizer:
    ...
    def token_type(self) -> enums.TokenTypeEnum:
        if enums.KeywordEnum.__members__.get(self.current_token.upper()):
            ...
            return enums.TokenTypeEnum.SYMBOL.lower()
        elif self.current_token[0].isalpha() or self.current_token[0] == "_":
            return enums.TokenTypeEnum.IDENTIFIER.lower()
        elif self.current_token[0] == "\"":
            return enums.TokenTypeEnum.STRING_CONST.lower()
        elif self.current_token[0].isnumeric():
            return enums.TokenTypeEnum.INT_CONST.lower()
        else:
            raise ValueError(f"Unhandled token {self.current_token}")        

    def keyword(self) -> enums.KeywordEnum:
        if self.token_type() != enums.TokenTypeEnum.KEYWORD.lower():
            raise ValueError("Should only be called for KEYWORDS")
        return self.current_token

    def symbol(self) -> str:
        if self.token_type() != enums.TokenTypeEnum.SYMBOL.lower():
    ...
    def identifier(self) -> str:
        if self.token_type() != enums.TokenTypeEnum.IDENTIFIER.lower():
            raise ValueError("Should only be called for IDENTIFIER")
        return self.current_token

    def int_val(self) -> int:
        if self.token_type() != enums.TokenTypeEnum.INT_CONST.lower():
            raise ValueError("Should only be called for INT_CONSTS")
        return self.current_token

    def string_val(self) -> str:
        if self.token_type() != enums.TokenTypeEnum.STRING_CONST.lower():
