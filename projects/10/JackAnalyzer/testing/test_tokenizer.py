...
import JackAnalyzer.src.enums as enums
import JackAnalyzer.src.tokenizer as tk
...
@pytest.mark.parametrize(
    "token,expected_token_type", [
        ('let', enums.TokenTypeEnum.KEYWORD),
        ('+', enums.TokenTypeEnum.SYMBOL),
        ('foo', enums.TokenTypeEnum.IDENTIFIER),
        ('"foo"', enums.TokenTypeEnum.STRING_CONST),
        ('3', enums.TokenTypeEnum.INT_CONST),
    ]
)
def test_token_type(
    tokenizer: tk.Tokenizer, token: str, expected_token_type: enums.TokenTypeEnum
):
