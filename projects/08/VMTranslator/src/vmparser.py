...
class VMParser:
    ...
    def advance(self) -> None:
        ...
        if not self._is_valid_line(loaded_line):
            ...
            if "//" in loaded_line:
                loaded_line = loaded_line.split("//")[0].strip()
            
            line_parts = loaded_line.split(" ")
            if len(line_parts) == 1:
                self._handle_1_part_instructions(line_parts)
            elif len(line_parts) == 2:
                self._handle_2_part_instructions(line_parts)
    ...
    def _handle_2_part_instructions(self, line_parts: list[str]) -> None:
        if line_parts[0] == "label":
            self.command_type = CommandTypeEnum.C_LABEL
            self.arg1 = line_parts[1]
            self.arg2 = None
        elif line_parts[0] == "goto":
            self.command_type = CommandTypeEnum.C_GOTO
            self.arg1 = line_parts[1]
            self.arg2 = None
        elif line_parts[0] == "if-goto":
            self.command_type = CommandTypeEnum.C_IF
            self.arg1 = line_parts[1]
            self.arg2 = None
        else:
            raise NotImplementedError(
